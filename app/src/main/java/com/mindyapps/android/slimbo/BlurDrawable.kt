...
class BlurDrawable(target: View?, radius: Int) : Drawable() {
    private val targetRef: WeakReference<View?>?
    private var blurred: Bitmap? = null
    private val paint: Paint?
    private var radius = 0

    constructor(target: View?) : this(target, 10) {}


    override fun draw(canvas: Canvas) {
        if (blurred == null) {
            val target: View = targetRef?.get()!!
            if (target != null) {
                val bitmap: Bitmap = target.getDrawingCache(true) ?: return
    ...
    override fun setBounds(bounds: Rect) {
        super.setBounds(-blurred!!.width / 2, blurred!!.height, blurred!!.width / 2, 0)
    }
    ...
    init {
        targetRef = WeakReference<View?>(target)
        setRadius(radius)
